...
class UsersController {
  ...
  if (!user || password !== user.password || status !== 1) {
    res.status(400).send({
  ...
  updateUser = async (req, res, next) => {
    ...
    const { nickname, password, phone, address, status, point } = req.body;

    const updateUserData = await this.userService.updateUser(nickname, password, phone, address, status, point);
